setwd("~/GitHub/LongMortalityTables/data-raw/2017CSO/CSO2017 Loaded Composite Gender Blended ANB")
...
CSO2017_Loaded_Composite_GenderBlend_ANB_Select <- bind_rows(all_select) %>%
...
CSO2017_Loaded_Composite_GenderBlend_ANB_Ultimate <- bind_rows(all_ultimate) %>%
  mutate(attained_age = as.integer(attained_age), q_ult = as.double(q_ult))

usethis::use_data(CSO2017_Loaded_Composite_GenderBlend_ANB_Select)
usethis::use_data(CSO2017_Loaded_Composite_GenderBlend_ANB_Ultimate)
readr::write_csv(CSO2017_Loaded_Composite_GenderBlend_ANB_Select, "~/GitHub/LongMortalityTables/final-data-csv/2017CSO/2017CSO Loaded Composite Gender Blended ANB/2017CSO Loaded Composite Gender Blended ANB Select.csv")
readr::write_csv(CSO2017_Loaded_Composite_GenderBlend_ANB_Ultimate, "~/GitHub/LongMortalityTables/final-data-csv/2017CSO/2017CSO Loaded Composite Gender Blended ANB/2017CSO Loaded Composite Gender Blended ANB Ultimate.csv")
...
combineSelectUltimate <- function(select, ultimate) {
  ...
  table_list <- ultimate %>%
    group_by(table) %>%
    summarise(gender_blend = max(gender_blend)) %>%
    mutate(dummy=TRUE)
  ...
  combined_frame %>%
    left_join(select, by = c("table", "gender_blend", "issue_age", "duration")) %>%
    left_join(ultimate, by = c("table", "gender_blend", "attained_age")) %>%
    select(-dummy) %>%
    arrange(table, issue_age, duration)
}

CSO2017_Loaded_Composite_GenderBlend_ANB_Combined <- combineSelectUltimate(CSO2017_Loaded_Composite_GenderBlend_ANB_Select, CSO2017_Loaded_Composite_GenderBlend_ANB_Ultimate)

usethis::use_data(CSO2017_Loaded_Composite_GenderBlend_ANB_Combined)
readr::write_csv(CSO2017_Loaded_Composite_GenderBlend_ANB_Combined, "~/GitHub/LongMortalityTables/final-data-csv/2017CSO/2017CSO Loaded Composite Gender Blended ANB/2017CSO Loaded Composite Gender Blended ANB Combined.csv")
